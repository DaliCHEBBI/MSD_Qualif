...
cImage cImage::FromString(const std::string & aName)
{
   Tiff_Im aTF = Tiff_Im::StdConvGen(aName,-1,true);
   Pt2di aSzIm = aTF.sz();
   cImage aRes(aSzIm.x,aSzIm.y);
...
void cImage::Save(const std::string & aName)
{
    Tiff_Im  aTF
             (
                 aName.c_str(),
...
void  cImage::SaveRGB(const std::string & aName,cImage & aI2,cImage & aI3)
{
    Tiff_Im  aTF
             (
                 aName.c_str(),
...
cImage  cImage::ImageMoy(int aSzW,int aNbIter)
{
   cImage aRes(mSz.x,mSz.y);
...
cImage  cImage::ImageReduite(double aFact)
{
   Pt2di aSzR = round_up(Pt2dr(mSz)/aFact);

   cImage aRes(aSzR.x,aSzR.y);
